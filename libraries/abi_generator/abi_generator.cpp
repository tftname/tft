#include <dncio/abi_generator/abi_generator.hpp>
...
string abi_generator::translate_type(const string& type_name) {
  ...
  if (type_name == "unsigned __int128" || type_name == "uint128_t") built_in_type = "uint128";
     ...
     static auto types = dncio::chain::common_type_defs();
     auto itr = std::find_if( types.begin(), types.end(),
                              [&type_name]( const dncio::chain::type_def& t ) { return t.new_type_name == type_name; } );
     if( itr != types.end()) {
...
void abi_generator::handle_decl(const Decl* decl) { try {
  ...
  // If dncIO_ABI macro was found, check if the current declaration
  ...
  // The current Decl was the type referenced in dncIO_ABI macro
  if( type_has_actions ) return;

  // The current Decl was not the type referenced in dncIO_ABI macro
  ...
  // If dncIO_ABI macro was found, we will only check if the current Decl
