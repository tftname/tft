...
#include <dncio/chain/resource_limits_private.hpp>
...
void resource_limits_manager::add_transaction_usage(const flat_set<account_name>& accounts, uint64_t cpu_usage, uint64_t net_usage, uint32_t time_slot ) {
   ...
   for( const auto& a : accounts ) {
      ...
      if( cpu_weight >= 0 && state.total_cpu_weight > 0 ) {
         ...
         dnc_ASSERT( cpu_used_in_window <= max_user_use_in_window,
      ...
      if( net_weight >= 0 && state.total_net_weight > 0) {
         ...
         dnc_ASSERT( net_used_in_window <= max_user_use_in_window,
   ...
   dnc_ASSERT( state.pending_cpu_usage <= config.cpu_limit_parameters.max, block_resource_exhausted, "Block has insufficient cpu resources" );
   dnc_ASSERT( state.pending_net_usage <= config.net_limit_parameters.max, block_resource_exhausted, "Block has insufficient net resources" );
}
...
void resource_limits_manager::add_pending_ram_usage( const account_name account, int64_t ram_delta ) {
   ...
   dnc_ASSERT( ram_delta <= 0 || UINT64_MAX - usage.ram_usage >= (uint64_t)ram_delta, transaction_exception,
              "Ram usage delta would overflow UINT64_MAX");
   dnc_ASSERT(ram_delta >= 0 || usage.ram_usage >= (uint64_t)(-ram_delta), transaction_exception,
...
void resource_limits_manager::verify_account_ram_usage( const account_name account )const {
   ...
   if( ram_bytes >= 0 ) {
      dnc_ASSERT( usage.ram_usage <= ram_bytes, ram_usage_exceeded,
...
bool resource_limits_manager::set_account_limits( const account_name& account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) {
   ...
   if( ram_bytes >= 0 ) {
      ...
      if( limits.ram_bytes < 0 ) {
         dnc_ASSERT(ram_bytes >= usage.ram_usage, wasm_execution_error, "converting unlimited account would result in overcommitment [commit=${c}, desired limit=${l}]", ("c", usage.ram_usage)("l", ram_bytes));
      } else {
         dnc_ASSERT(ram_bytes >= usage.ram_usage, wasm_execution_error, "attempting to release committed ram resources [commit=${c}, desired limit=${l}]", ("c", usage.ram_usage)("l", ram_bytes));
      }
      */
   }
...
void resource_limits_manager::process_account_limit_updates() {
   ...
   auto update_state_and_value = [](uint64_t &total, int64_t &value, int64_t pending_value, const char* debug_which) -> void {
      if (value > 0) {
         dnc_ASSERT(total >= value, rate_limiting_state_inconsistent, "underflow when reverting old value to ${which}", ("which", debug_which));
         total -= value;
      }

      if (pending_value > 0) {
         dnc_ASSERT(UINT64_MAX - total >= pending_value, rate_limiting_state_inconsistent, "overflow when applying new value to ${which}", ("which", debug_which));
         total += pending_value;
      }
