...
#include <dncio/chain/authority_checker.hpp>
...
namespace dncio { namespace chain {
   ...
   } dnc_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }
                                                                             ...
                                                                             )const
   {
      ...
      try {
         ...
         if( *linked_permission == config::dncio_any_name )
            return optional<permission_name>();
   ...
   {
      dnc_ASSERT( auths.size() == 1, irrelevant_auth_exception,
                  "updateauth action should only have one declared authorization" );
      const auto& auth = auths[0];
      dnc_ASSERT( auth.actor == update.account, irrelevant_auth_exception,
      ...
      dnc_ASSERT( get_permission(auth).satisfies( *min_permission,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      dnc_ASSERT( auth.actor == del.account, irrelevant_auth_exception,
                                                           ...
                                                           )const
   {
      ...
      const auto& auth = auths[0];
      dnc_ASSERT( auth.actor == link.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      dnc_ASSERT( link.type != updateauth::get_name(),  action_validate_exception,
                  "Cannot link dncio::updateauth to a minimum permission" );
      dnc_ASSERT( link.type != deleteauth::get_name(),  action_validate_exception,
                  "Cannot link dncio::deleteauth to a minimum permission" );
      dnc_ASSERT( link.type != linkauth::get_name(),    action_validate_exception,
                  "Cannot link dncio::linkauth to a minimum permission" );
      dnc_ASSERT( link.type != unlinkauth::get_name(),  action_validate_exception,
                  "Cannot link dncio::unlinkauth to a minimum permission" );
      dnc_ASSERT( link.type != canceldelay::get_name(), action_validate_exception,
                  "Cannot link dncio::canceldelay to a minimum permission" );

      const auto linked_permission_name = lookup_minimum_permission(link.account, link.code, link.type);

      if( !linked_permission_name ) // if action is linked to dncio.any permission
         return;

      dnc_ASSERT( get_permission(auth).satisfies( get_permission({link.account, *linked_permission_name}),
                                                  _db.get_index<permission_index>().indices()              ),
                  irrelevant_auth_exception,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      dnc_ASSERT( auth.actor == unlink.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      const auto unlinked_permission_name = lookup_linked_permission(unlink.account, unlink.code, unlink.type);
      dnc_ASSERT( unlinked_permission_name.valid(), transaction_exception,
      ...
      if( *unlinked_permission_name == config::dncio_any_name )
         return;

      dnc_ASSERT( get_permission(auth).satisfies( get_permission({unlink.account, *unlinked_permission_name}),
                                                  _db.get_index<permission_index>().indices()                  ),
                  irrelevant_auth_exception,
                                                                          ...
                                                                          )const
   {
      ...
      dnc_ASSERT( get_permission(auth).satisfies( get_permission(cancel.canceling_auth),
                                                  _db.get_index<permission_index>().indices() ),
                  irrelevant_auth_exception,
      ...
      dnc_ASSERT( found, action_validate_exception,
                                             ...
                                             )const
   {
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is dncio.any
                  const auto& min_permission = get_permission({declared_auth.actor, *min_permission_name});
                  dnc_ASSERT( get_permission(declared_auth).satisfies( min_permission,
                                                                       _db.get_index<permission_index>().indices() ),
                              irrelevant_auth_exception,
      ...
      for( const auto& p : permissions_to_satisfy ) {
         ...
         dnc_ASSERT( checker.satisfied( p.first, p.second ), unsatisfied_authorization,
      ...
      if( !allow_unused_keys ) {
         dnc_ASSERT( checker.all_keys_used(), tx_irrelevant_sig,
                                             ...
                                             )const
   {
      ...
      dnc_ASSERT( checker.satisfied({account, permission}), unsatisfied_authorization,
                                                                     ...
                                                                     )const
   {
      ...
      for (const auto& act : trx.actions ) {
         for (const auto& declared_auth : act.authorization) {
            dnc_ASSERT( checker.satisfied(declared_auth), unsatisfied_authorization,
