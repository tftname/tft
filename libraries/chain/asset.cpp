...
void asset::reflector_verify()const {
  dnc_ASSERT( is_amount_within_range(), asset_type_exception, "magnitude of asset amount must be less than 2^62" );
  dnc_ASSERT( sym.valid(), asset_type_exception, "invalid symbol" );
}
...
asset asset::from_string(const string& from)
{
   try {
      ...
      dnc_ASSERT((space_pos != string::npos), asset_type_exception, "Asset's amount and symbol should be separated with space");
      ...
      if (dot_pos != string::npos) {
         dnc_ASSERT((dot_pos != amount_str.size() - 1), asset_type_exception, "Missing decimal fraction after decimal point");
      }

      // Parse symbol
      string precision_digit_str;
      if (dot_pos != string::npos) {
         precision_digit_str = dncio::chain::to_string(amount_str.size() - dot_pos - 1);
      } else {
