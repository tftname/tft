...
namespace dncio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(dncio);
const static uint64_t null_account_name      = N(dncio.null);
const static uint64_t producers_account_name = N(dncio.prods);
...
const static uint64_t dncio_auth_scope       = N(dncio.auth);
const static uint64_t dncio_all_scope        = N(dncio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t dncio_any_name = N(dncio.any);
const static uint64_t dncio_code_name = N(dncio.code);
...
const static dncio::chain::wasm_interface::vm_type default_wasm_runtime = dncio::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace dncio::chain::config

constexpr uint64_t dnc_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / dncio::chain::config::percent_100;
}

template<typename Number>
Number dnc_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + dncio::chain::config::percent_100 - dncio::chain::config::percent_1)  / dncio::chain::config::percent_100;
}
