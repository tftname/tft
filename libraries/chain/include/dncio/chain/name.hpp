...
#define N(X) dncio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<dncio::chain::name> : private hash<uint64_t> {
      typedef dncio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const dncio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, dncio::chain::name& check);
} // fc


FC_REFLECT( dncio::chain::name, (value) )
