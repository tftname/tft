#pragma once
#include <dncio/chain/block_header.hpp>
...
FC_REFLECT_ENUM( dncio::chain::transaction_receipt::status_enum,
                 (executed)(soft_fail)(hard_fail)(delayed)(expired) )

FC_REFLECT(dncio::chain::transaction_receipt_header, (status)(cpu_usage_us)(net_usage_words) )
FC_REFLECT_DERIVED(dncio::chain::transaction_receipt, (dncio::chain::transaction_receipt_header), (trx) )
FC_REFLECT_DERIVED(dncio::chain::signed_block, (dncio::chain::signed_block_header), (transactions)(block_extensions) )
