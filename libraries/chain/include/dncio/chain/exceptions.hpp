...
#include <dncio/chain/protocol.hpp>
#include <boost/core/typeinfo.hpp>


#define dnc_ASSERT( expr, exc_type, FORMAT, ... )                \
   FC_MULTILINE_MACRO_BEGIN                                           \
   if( !(expr) )                                                      \
      FC_THROW_EXCEPTION( exc_type, FORMAT, __VA_ARGS__ );            \
   FC_MULTILINE_MACRO_END

#define dnc_THROW( exc_type, FORMAT, ... ) \
    throw exc_type( FC_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) );

/**
 * Macro inspired from FC_RETHROW_EXCEPTIONS
 * The main difference here is that if the exception caught isn't of type "dncio::chain::chain_exception"
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define dnc_RETHROW_EXCEPTIONS(exception_type, FORMAT, ... ) \
   catch (dncio::chain::chain_exception& e) { \
 ...
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define dnc_CAPTURE_AND_RETHROW( exception_type, ... ) \
   catch (dncio::chain::chain_exception& e) { \
...
#define dnc_RECODE_EXC( cause_type, effect_type ) \
...
namespace dncio { namespace chain {
   ...
   FC_DECLARE_DERIVED_EXCEPTION( action_validate_exception, chain_exception,
      ...
      FC_DECLARE_DERIVED_EXCEPTION( dncio_assert_message_exception, action_validate_exception,
                                    3050003, "dncio_assert_message assertion failure" )
      FC_DECLARE_DERIVED_EXCEPTION( dncio_assert_code_exception, action_validate_exception,
                                    3050004, "dncio_assert_code assertion failure" )
