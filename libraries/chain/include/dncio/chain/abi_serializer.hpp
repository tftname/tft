...
namespace impl {
   ...
   struct abi_from_variant {
      ...
      {
         const variant_object& vo = v.get_object();
         dnc_ASSERT(vo.contains("account"), packed_transaction_type_exception, "Missing account");
         dnc_ASSERT(vo.contains("name"), packed_transaction_type_exception, "Missing name");
         ...
         dnc_ASSERT(!act.data.empty(), packed_transaction_type_exception,
      ...
      static void extract( const variant& v, packed_transaction& ptrx, Resolver resolver ) {
         const variant_object& vo = v.get_object();
         dnc_ASSERT(vo.contains("signatures"), packed_transaction_type_exception, "Missing signatures");
         dnc_ASSERT(vo.contains("compression"), packed_transaction_type_exception, "Missing compression");
         ...
         if( vo.contains("packed_trx") && vo["packed_trx"].is_string() && !vo["packed_trx"].as_string().empty() ) {
            ...
            dnc_ASSERT(vo.contains("transaction"), packed_transaction_type_exception, "Missing transaction");
            transaction trx;
            vector<bytes> context_free_data;
