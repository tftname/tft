...
namespace dncio { namespace chain { namespace resource_limits {
   ...
   namespace impl {
      ...
      struct exponential_moving_average_accumulator
      {
         ...
         void add( uint64_t units, uint32_t ordinal, uint32_t window_size /* must be positive */ )
         {
            ...
            dnc_ASSERT(units <= max_raw_value, rate_limiting_state_inconsistent, "Usage exceeds maximum value representable after extending for precision");
            dnc_ASSERT(std::numeric_limits<decltype(consumed)>::max() - consumed >= units, rate_limiting_state_inconsistent, "Overflow in tracked usage when adding usage!");

            auto value_ex_contrib = integer_divide_ceil(units * Precision, (uint64_t)window_size);
            dnc_ASSERT(std::numeric_limits<decltype(value_ex)>::max() - value_ex >= value_ex_contrib, rate_limiting_state_inconsistent, "Overflow in accumulated value when adding usage!");
   ...
   class resource_limits_config_object : public chainbase::object<resource_limits_config_object_type, resource_limits_config_object> {
      ...
      elastic_limit_parameters cpu_limit_parameters = {dnc_PERCENT(config::default_max_block_cpu_usage, config::default_target_block_cpu_usage_pct), config::default_max_block_cpu_usage, config::block_cpu_usage_average_window_ms / config::block_interval_ms, 1000, {99, 100}, {1000, 999}};
      elastic_limit_parameters net_limit_parameters = {dnc_PERCENT(config::default_max_block_net_usage, config::default_target_block_net_usage_pct), config::default_max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, 1000, {99, 100}, {1000, 999}};
...
} } } /// dncio::chain::resource_limits

CHAINBASE_SET_INDEX_TYPE(dncio::chain::resource_limits::resource_limits_object,        dncio::chain::resource_limits::resource_limits_index)
CHAINBASE_SET_INDEX_TYPE(dncio::chain::resource_limits::resource_usage_object,         dncio::chain::resource_limits::resource_usage_index)
CHAINBASE_SET_INDEX_TYPE(dncio::chain::resource_limits::resource_limits_config_object, dncio::chain::resource_limits::resource_limits_config_index)
CHAINBASE_SET_INDEX_TYPE(dncio::chain::resource_limits::resource_limits_state_object,  dncio::chain::resource_limits::resource_limits_state_index)
