...
#include <dncio/chain/core_symbol.hpp>
#include <string>
#include <functional>
...
namespace dncio {
   namespace chain {
      ...
      /**
         ...
         from_string constructs a symbol from an input a string of the form "4,dnc"
...
#define SY(P,X) ::dncio::chain::string_to_symbol_c(P,#X)
      ...
      class symbol {
         public:
            ...
            explicit operator string() const
            {
               uint64_t v = m_value;
               uint8_t p = v & 0xFF;
               string ret = dncio::chain::to_string(p);
               ret += ',';
               ret += name();
               return ret;
            }
...
namespace fc {
   inline void to_variant(const dncio::chain::symbol& var, fc::variant& vo) { vo = var.to_string(); }
   inline void from_variant(const fc::variant& var, dncio::chain::symbol& vo) {
      vo = dncio::chain::symbol::from_string(var.get_string());
   }
}

namespace fc {
   inline void to_variant(const dncio::chain::symbol_code& var, fc::variant& vo) {
      vo = dncio::chain::symbol(var.value << 8).name();
   }
   inline void from_variant(const fc::variant& var, dncio::chain::symbol_code& vo) {
      vo = dncio::chain::symbol(0, var.get_string().c_str()).to_symbol_code();
   }
}

FC_REFLECT(dncio::chain::symbol_code, (value))
FC_REFLECT(dncio::chain::symbol, (m_value))
FC_REFLECT(dncio::chain::extended_symbol, (sym)(contract))
