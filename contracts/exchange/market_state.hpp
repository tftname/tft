...
namespace dncio {
   ...
   struct margin_position {
      ...
      dncLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef dncio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), dncio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      dncLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef dncio::multi_index<N(loans), loan_position> loans;
