...
class dice : public dncio::contract {
   public:
      ...
      void offerbet(const asset& bet, const account_name player, const checksum256& commitment) {
         ...
         dncio_assert( bet.symbol == CORE_SYMBOL, "only core token allowed" );
         dncio_assert( bet.is_valid(), "invalid bet" );
         dncio_assert( bet.amount > 0, "must bet positive quantity" );

         dncio_assert( !has_offer( commitment ), "offer with this commitment already exist" );
         require_auth( player );

         auto cur_player_itr = accounts.find( player );
         dncio_assert(cur_player_itr != accounts.end(), "unknown account");
         ...
         if( matched_offer_itr == idx.end()
            ...
            accounts.modify( cur_player_itr, 0, [&](auto& acnt) {
               dncio_assert( acnt.dnc_balance >= bet, "insufficient balance" );
               acnt.dnc_balance -= bet;
               acnt.open_offers++;
            });
            ...
            auto game_itr = games.emplace(_self, [&](auto& new_game){
               ...
               new_game.deadline = dncio::time_point_sec(0);
      ...
      void canceloffer( const checksum256& commitment ) {
         ...
         dncio_assert( offer_itr != idx.end(), "offer does not exists" );
         dncio_assert( offer_itr->gameid == 0, "unable to cancel offer" );
         require_auth( offer_itr->owner );

         auto acnt_itr = accounts.find(offer_itr->owner);
         accounts.modify(acnt_itr, 0, [&](auto& acnt){
            acnt.open_offers--;
            acnt.dnc_balance += offer_itr->bet;
         });
      ...
      void reveal( const checksum256& commitment, const checksum256& source ) {
         ...
         dncio_assert(curr_revealer_offer != idx.end(), "offer not found");
         dncio_assert(curr_revealer_offer->gameid > 0, "unable to reveal");
         ...
         dncio_assert( is_zero(curr_reveal.reveal) == true, "player already revealed");
         ...
         if( !is_zero(prev_reveal.reveal) ) {
            ...
            games.modify(game_itr, 0, [&](auto& game){
               ...
               game.deadline = dncio::time_point_sec(now() + FIVE_MINUTES);
            });
         }
      }
      ...
      void claimexpired( const uint64_t gameid ) {
         ...
         dncio_assert(game_itr != games.end(), "game not found");
         dncio_assert(game_itr->deadline != dncio::time_point_sec(0) && dncio::time_point_sec(now()) > game_itr->deadline, "game not expired");
         ...
         if( !is_zero(game_itr->player1.reveal) ) {
            dncio_assert( is_zero(game_itr->player2.reveal), "game error");
            pay_and_clean(*game_itr, *player1_offer, *player2_offer);
         } else {
            dncio_assert( is_zero(game_itr->player1.reveal), "game error");
      ...
      void deposit( const account_name from, const asset& quantity ) {
         ...
         dncio_assert( quantity.amount > 0, "must deposit positive quantity" );
         ...
         action(
            ...
            N(dncio.token), N(transfer),
            std::make_tuple(from, _self, quantity, std::string(""))
         ).send();

         accounts.modify( itr, 0, [&]( auto& acnt ) {
            acnt.dnc_balance += quantity;
         });
      }
      ...
      void withdraw( const account_name to, const asset& quantity ) {
         ...
         dncio_assert( quantity.is_valid(), "invalid quantity" );
         dncio_assert( quantity.amount > 0, "must withdraw positive quantity" );

         auto itr = accounts.find( to );
         dncio_assert(itr != accounts.end(), "unknown account");

         accounts.modify( itr, 0, [&]( auto& acnt ) {
            dncio_assert( acnt.dnc_balance >= quantity, "insufficient balance" );
            acnt.dnc_balance -= quantity;
         });
   ...
   private:
      ...
      struct offer {
         ...
         dncLIB_SERIALIZE( offer, (id)(owner)(bet)(commitment)(gameid) )
      };

      typedef dncio::multi_index< N(offer), offer,
      ...
      struct player {
         ...
         dncLIB_SERIALIZE( player, (commitment)(reveal) )
      };
      ...
      struct game {
         uint64_t id;
         asset    bet;
         dncio::time_point_sec deadline;
         player   player1;
         player   player2;

         uint64_t primary_key()const { return id; }

         dncLIB_SERIALIZE( game, (id)(bet)(deadline)(player1)(player2) )
      };

      typedef dncio::multi_index< N(game), game> game_index;
      ...
      struct global_dice {
         ...
         dncLIB_SERIALIZE( global_dice, (id)(nextgameid) )
      };

      typedef dncio::multi_index< N(global), global_dice> global_dice_index;
      ...
      struct account {
         ...
         bool is_empty()const { return !( dnc_balance.amount | open_offers | open_games ); }

         uint64_t primary_key()const { return owner; }

         dncLIB_SERIALIZE( account, (owner)(dnc_balance)(open_offers)(open_games) )
      };

      typedef dncio::multi_index< N(account), account> account_index;
...
dncIO_ABI( dice, (offerbet)(canceloffer)(reveal)(claimexpired)(deposit)(withdraw) )
