...
## dncio.system::cfgproducer    account config
...
## dncio.system::okproducer     account producer vote
...
## dncio.system::setproxy       account proxy
...
## dncio.system::unstake        account quantity 
...
## dncio.system::withdraw account
   - this action can only be triggered by dncio.system via a deferred transaction generated by unstake
   - this will generate an inline dncio.token::transfer call from=dncio.system to=account and amount equal to the next withdraw increment
   - this will generate another deferred withdraw to continue the process if there are still withdraws to be made


## dncio.system::transfer from to amount memo
   - decrements balance of from if amount <= balance
   - increments balance of to by amount
   - memo is ignored 

## dncio.system::stakevote account amount
   - the primary currency of dncio is controlled by the token contract which enables users to transfer
...
## dncio.system::onblock   account blocktime blocknum
...
## dncio.system::freeze producer accounts true|false 
...
## dncio.system::paystandby producer
