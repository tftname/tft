...
namespace dnciosystem {
   using dncio::asset;
   using dncio::indexed_by;
   using dncio::const_mem_fun;
   using dncio::bytes;
   using dncio::print;
   using dncio::permission_level;
   using std::map;
   using std::pair;
   ...
   struct user_resources {
      ...
      dncLIB_SERIALIZE( user_resources, (owner)(net_weight)(cpu_weight)(ram_bytes) )
   };
   ...
   struct delegated_bandwidth {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      dncLIB_SERIALIZE( delegated_bandwidth, (from)(to)(net_weight)(cpu_weight) )
   ...
   struct refund_request {
      account_name  owner;
      time          request_time;
      dncio::asset  net_amount;
      dncio::asset  cpu_amount;

      uint64_t  primary_key()const { return owner; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      dncLIB_SERIALIZE( refund_request, (owner)(request_time)(net_amount)(cpu_amount) )
   };
    ...
    */
   typedef dncio::multi_index< N(userres), user_resources>      user_resources_table;
   typedef dncio::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef dncio::multi_index< N(refunds), refund_request>      refunds_table;
   ...
   void system_contract::buyrambytes( account_name payer, account_name receiver, uint32_t bytes ) {
      ...
      auto tmp = *itr;
      auto dncout = tmp.convert( asset(bytes,S(0,RAM)), CORE_SYMBOL );

      buyram( payer, receiver, dncout );
   }
   ...
   {
      require_auth( payer );
      dncio_assert( quant.amount > 0, "must purchase a positive amount" );
      ...
      INLINE_ACTION_SENDER(dncio::token, transfer)( N(dncio.token), {payer,N(active)},
         { payer, N(dncio.ram), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(dncio::token, transfer)( N(dncio.token), {payer,N(active)},
                                                       { payer, N(dncio.ramfee), fee, std::string("ram fee") } );
      }
      ...
      dncio_assert( bytes_out > 0, "must reserve a positive amount" );
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      dncio_assert( bytes > 0, "cannot sell negative byte" );

      user_resources_table  userres( _self, account );
      auto res_itr = userres.find( account );
      dncio_assert( res_itr != userres.end(), "no resource row" );
      dncio_assert( res_itr->ram_bytes >= bytes, "insufficient quota" );
      ...
      dncio_assert( _gstate.total_ram_stake >= 0, "error, attempt to unstake more tokens than previously staked" );
      ...
      INLINE_ACTION_SENDER(dncio::token, transfer)( N(dncio.token), {N(dncio.ram),N(active)},
                                                       { N(dncio.ram), account, asset(tokens_out), std::string("sell ram") } );
      auto fee = tokens_out.amount / 200;
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(dncio::token, transfer)( N(dncio.token), {account,N(active)},
            { account, N(dncio.ramfee), asset(fee), std::string("sell ram fee") } );
      }
   }
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      dncio_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
   ...
   {
      require_auth( from );
      dncio_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         dncio_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         dncio_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         dncio_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         dncio_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(dncio.stake) != source_stake_from ) { //for dncio both transfer and refund make no sense
         ...
         if ( req != refunds_tbl.end() ) { //need to update refund
            ...
            dncio_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
            dncio_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(dncio::token, transfer)( N(dncio.token), {source_stake_from, N(active)},
               { source_stake_from, N(dncio.stake), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
      ...
      // update voting power
      {
         ...
         dncio_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
   ...
   {
      dncio_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      dncio_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      dncio_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
   ...
   {
      dncio_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      dncio_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      dncio_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
      dncio_assert( _gstate.total_activated_stake >= min_activated_stake, "not enough has been staked for users to unstake" );
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      dncio_assert( req != refunds_tbl.end(), "refund request not found" );
      dncio_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
      // Until now() becomes NOW, the fact that now() is the timestamp of the previous block could in theory
      // allow people to get their tokens earlier than the 3 day delay if the unstake happened immediately after many
      // consecutive missed blocks.

      INLINE_ACTION_SENDER(dncio::token, transfer)( N(dncio.token), {N(dncio.stake),N(active)},
                                                    { N(dncio.stake), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
