...
namespace dncio {
    ...
    *    int  c; //4
    *
    *    dncLIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   struct permission_level {
      ...
      dncLIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      dncLIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         dncio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         dncio_assert( name == T::get_name(), "Invalid name" );
         dncio_assert( account == T::get_account(), "Invalid account" );
...
::dncio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::dncio::string_to_name(#NAME) )
...
#define ACTION( CODE, NAME ) struct NAME : ::dncio::action_meta<CODE, ::dncio::string_to_name(#NAME) >
