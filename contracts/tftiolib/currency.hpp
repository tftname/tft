   ...
   class currency {
      public:
         ...
         struct create {
           ...
           // array<char,32>         issuer_agreement_hash;
            ...
            dncLIB_SERIALIZE( create, (issuer)(maximum_supply)(issuer_can_freeze)(issuer_can_recall)(issuer_can_whitelist) )
         };
         ...
         struct transfer
         {
            ...
            dncLIB_SERIALIZE( transfer, (from)(to)(quantity)(memo) )
         };
         ...
         struct issue {
            ...
            dncLIB_SERIALIZE( issue, (to)(quantity)(memo) )
         };
         ...
         struct fee_schedule {
            ...
            dncLIB_SERIALIZE( fee_schedule, (fee_per_length) )
         };
         ...
         struct account {
            ...
            dncLIB_SERIALIZE( account, (balance)(frozen)(whitelist) )
         };
         ...
         struct currency_stats {
            ...
            dncLIB_SERIALIZE( currency_stats, (supply)(max_supply)(issuer)(can_freeze)(can_recall)(can_whitelist)(is_frozen)(enforce_whitelist) )
         };

         typedef dncio::multi_index<N(accounts), account> accounts;
         typedef dncio::multi_index<N(stat), currency_stats> stats;
         ...
         bool apply( account_name contract, action_name act ) {
          ...
          void issue_currency( const issue& i ) {
             ...
             statstable.modify( st, 0, [&]( auto& s ) {
                ...
                dncio_assert( s.supply.amount >= 0, "underflow" );
             });
          ...
          void on( const issue& i ) {
             ...
             dncio_assert( i.quantity.is_valid(), "invalid quantity" );
             dncio_assert( i.quantity.amount > 0, "must issue positive quantity" );
          ...
          void on( const transfer& t ) {
             ...
             dncio_assert( t.quantity.is_valid(), "invalid quantity" );
             dncio_assert( t.quantity.amount > 0, "must transfer positive quantity" );
      ...
      private:
          void sub_balance( account_name owner, asset value, const currency_stats& st ) {
             ...
             if( has_auth( owner ) ) {
                dncio_assert( !st.can_freeze || !from.frozen, "account is frozen by issuer" );
                dncio_assert( !st.can_freeze || !st.is_frozen, "all transfers are frozen by issuer" );
                dncio_assert( !st.enforce_whitelist || from.whitelist, "account is not white listed" );
             } else if( has_auth( st.issuer ) ) {
                dncio_assert( st.can_recall, "issuer may not recall token" );
             } else {
                dncio_assert( false, "insufficient authority" );
             }
          ...
          void add_balance( account_name owner, asset value, const currency_stats& st, account_name ram_payer )
          {
             ...
             if( to == to_acnts.end() ) {
                dncio_assert( !st.enforce_whitelist, "can only transfer to white listed accounts" );
                to_acnts.emplace( ram_payer, [&]( auto& a ){
                  a.balance = value;
                });
             } else {
                dncio_assert( !st.enforce_whitelist || to->whitelist, "receiver requires whitelist by issuer" );
