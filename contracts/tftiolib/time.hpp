...
namespace dncio {
  class microseconds {
    public:
        ...
        int64_t _count;
        dncLIB_SERIALIZE( microseconds, (_count) )
    private:
  ...
  class time_point {
    public:
        ...
        dncLIB_SERIALIZE( time_point, (elapsed) )
  };
  ...
  class time_point_sec
  {
    public:
        ...
        operator time_point()const { return time_point( dncio::seconds( utc_seconds) ); }
        uint32_t sec_since_epoch()const { return utc_seconds; }

        time_point_sec operator = ( const dncio::time_point& t )
        {
          utc_seconds = uint32_t(t.time_since_epoch().count() / 1000000ll);
          return *this;
        }
        ...
        dncLIB_SERIALIZE( time_point_sec, (utc_seconds) )
  };
   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            dncio_assert( std::numeric_limits<uint32_t>::max() - slot >= 1, "block timestamp overflow" );
         ...
         dncLIB_SERIALIZE( block_timestamp, (slot) )
      private:
