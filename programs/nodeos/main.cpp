/**
 *  @file
 *  @copyright defined in dncio/LICENSE.txt
 */
#include <appbase/application.hpp>

#include <dncio/chain_plugin/chain_plugin.hpp>
#include <dncio/http_plugin/http_plugin.hpp>
...
{
   try {
      app().set_version(dncio::noddnc::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "dncio/noddnc/data" );
      app().set_default_config_dir(root / "dncio/noddnc/config" );
      if(!app().initialize<chain_plugin, http_plugin, net_plugin, producer_plugin>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("noddnc version ${ver}", ("ver", dncio::utilities::common::itoh(static_cast<uint32_t>(app().version()))));
      ilog("dncio root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
