 ...
 *  @defgroup dncclienttool dncIO Command Line Client Reference
 *  @brief Tool for sending transactions and querying state from @ref noddnc
 *  @ingroup dncclienttool
 */

/**
  @defgroup dncclienttool

  @section intro Introduction to cldnc

  `cldnc` is a command line tool that interfaces with the REST api exposed by @ref noddnc. In order to use `cldnc` you will need to
  have a local copy of `noddnc` running and configured to load the 'dncio::chain_api_plugin'.

   cldnc contains documentation for all of its commands. For a list of all commands known to cldnc, simply run it with no arguments:
```
$ ./cldnc
Command Line Interface to dncIO Client
Usage: programs/cldnc/cldnc [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where noddnc is running
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where kdncd is running
...
Subcommands:
  ...
  transfer                    Transfer dnc from account to account
...
Usage: ./cldnc create SUBCOMMAND

Subcommands:
  key                         Create a new keypair and print the public and private keys
  account                     Create a new account on the blockchain

$ ./cldnc create account
Create a new account on the blockchain
Usage: ./cldnc create account [OPTIONS] creator name OwnerKey ActiveKey
...
#include <dncio/chain/name.hpp>
#include <dncio/chain/config.hpp>
#include <dncio/chain/wast_to_wasm.hpp>
#include <dncio/chain/trace.hpp>
...
using namespace dncio::utilities;
using namespace dncio::client::help;
using namespace dncio::client::http;
using namespace dncio::client::localize;
using namespace dncio::client::config;
using namespace boost::filesystem;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define dncC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      dncio::client::http::connection_param *cp = new dncio::client::http::connection_param((std::string&)url, (std::string&)path,
              no_verify ? false : true, headers);

      return dncio::client::http::do_http_call( *cp, fc::variant(v), print_request );
   }
   catch(boost::system::system_error& e) {
      if(url == ::url)
         std::cerr << localized("Failed to connect to noddnc at ${u}; is noddnc running?", ("u", url)) << std::endl;
      else if(url == ::wallet_url)
         std::cerr << localized("Failed to connect to kdncd at ${u}; is kdncd running?", ("u", url)) << std::endl;
...
dncio::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<dncio::chain_apis::read_only::get_info_results>();
}
...
fc::variant push_transaction( signed_transaction& trx, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   } dnc_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active) {
   return action {
      ...
      dncio::chain::newaccount{
         .creator      = creator,
         .name         = newaccount,
         .owner        = dncio::chain::authority{1, {{owner, 1}}, {}},
         .active       = dncio::chain::authority{1, {{active, 1}}, {}}
      }
   };
}
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } dnc_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "dnc")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } dnc_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
...
asset to_asset( const string& code, const string& s ) {
   static map<dncio::chain::symbol_code, dncio::chain::symbol> cache;
   auto a = asset::from_string( s );
   dncio::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<dncio::chain_apis::read_only::get_currency_stats_result>();
...
void try_port( uint16_t port, uint32_t duration ) {
   ...
   while ( !port_used(port)) {
      if (duration_cast<std::chrono::milliseconds>( system_clock::now().time_since_epoch()).count() - start_time > duration ) {
         std::cerr << "Unable to connect to kdncd, if kdncd is running please kill the process and try again.\n";
         throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to kdncd")});
      }
   }
}

void ensure_kdncd_running(CLI::App* app) {
    // get, version, net do not require kdncd
    ...
    if (port_used(uint16_t(wallet_port)))  // Hopefully taken by kdncd
        return;


    boost::filesystem::path binPath = boost::dll::program_location();
    binPath.remove_filename();
    // This extra check is necessary when running cldnc like this: ./cldnc ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append("kdncd"); // if cldnc and kdncd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kdncd").append("kdncd");
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child kdnc(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (kdnc.running()) {
        ...
        std::cerr << "No wallet service listening on 127.0.0.1: " << std::to_string(wallet_port)
                  << ". Cannot automatically start kdncd because kdncd was not found." << std::endl;
    }
}
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->set_callback([this] {
         ...
         } dnc_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of dnc delegated for net bandwidth")))->required();
         createAccount->add_option("--stake-cpu", stake_cpu,
                                   (localized("The amount of dnc delegated for CPU bandwidth")))->required();
         ...
         createAccount->add_option("--buy-ram", buy_ram_dnc,
                                   (localized("The amount of RAM bytes to purchase for the new account in dnc")));
         createAccount->add_flag("--transfer", transfer,
                                 (localized("Transfer voting power and right to unstake dnc to receiver")));
      }
      ...
      createAccount->set_callback([this] {
            ...
            } dnc_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str));
            try {
               active_key = public_key_type(active_key_str);
            } dnc_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str));
            auto create = create_newaccount(creator, account_name, owner_key, active_key);
            if (!simple) {
               if ( buy_ram_dnc.empty() && buy_ram_bytes_in_kbytes == 0) {
                  std::cerr << "ERROR: Either --buy-ram or --buy-ram-kbytes with non-zero value is required" << std::endl;
                  return;
               }
               action buyram = !buy_ram_dnc.empty() ? create_buyram(creator, account_name, to_asset(buy_ram_dnc))
...
struct vote_producers_subcommand {
   string voter_str;
   vector<dncio::name> producer_names;
...
struct approve_producer_subcommand {
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            ...
            auto res = result.as<dncio::chain_apis::read_only::get_table_rows_result>();
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<dncio::chain_apis::read_only::get_producers_result>();
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of dnc to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of dnc to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake dnc to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of dnc to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of dnc to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of dnc to bid"))->required();
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "dncio")("scope", "dncio")("table", "namebids")
                               ("lower_bound", dncio::chain::string_to_name(newname_str.c_str()))("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         }
         auto result = rawResult.as<dncio::chain_apis::read_only::get_table_rows_result>();
         if ( result.rows.empty() ) {
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("tokens", amount, localized("The amount of dnc to pay for RAM"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive dnc for sold RAM"))->required();
...
void get_account( const string& accountName, bool json_format ) {
   ...
   auto res = json.as<dncio::chain_apis::read_only::get_account_results>();
   ...
   if (!json_format) {
      ...
      unordered_map<name, dncio::chain_apis::permission> cache;
...
int main( int argc, char** argv ) {
   ...
   CLI::App app{"Command Line Interface to dncIO Client"};
   app.require_subcommand();
   app.add_option( "-H,--host", obsoleted_option_host_port, localized("the host where noddnc is running") )->group("hidden");
   app.add_option( "-p,--port", obsoleted_option_host_port, localized("the port where noddnc is running") )->group("hidden");
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where kdncd is running") )->group("hidden");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where kdncd is running") )->group("hidden");

   app.add_option( "-u,--url", url, localized("the http/https URL where noddnc is running"), true );
   app.add_option( "--wallet-url", wallet_url, localized("the http/https URL where kdncd is running"), true );

   app.add_option( "-r,--header", header_opt_callback, localized("pass specific HTTP header; repeat this option to pass multiple headers"));
   app.add_flag( "-n,--no-verify", no_verify, localized("don't verify peer certificate when using HTTPS"));
   app.set_callback([&app]{ ensure_kdncd_running(&app);});
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", dncio::client::config::version_str)) << std::endl;
   });
   ...
   getAccounts->set_callback([&] {
      ...
      } dnc_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
   ...
   getTransaction->set_callback([&] {
      ...
      } dnc_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", transaction_id_str))
...
//   int32_t offset = -20;
   ...
   getTransactions->set_callback([&] {
      ...
      if (skip_seq_str.empty()) {
         ...
         } dnc_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Skip Seq: ${skip_seq}", ("skip_seq", skip_seq_str))
         if (num_seq_str.empty()) {
            ...
            } dnc_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Num Seq: ${num_seq}", ("num_seq", num_seq_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_abi_callback = [&]() {
      ...
      } dnc_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
      if ( shouldSend ) {
   ...
   // Transfer subcommand
   string con = "dncio.token";
   string sender;
   string recipient;
   string amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer dnc from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending dnc"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving dnc"))->required();
   transfer->add_option("amount", amount, localized("The amount of dnc to send"))->required();
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         wallet_key = private_key_type( wallet_key_str );
          ...
          dnc_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   auto stopKdncd = wallet->add_subcommand("stop", localized("Stop kdncd (doesn't work with noddnc)."), false);
   stopKdncd->set_callback([] {
      // wait for kdncd to come up
      try_port(uint16_t(std::stoi(parse_url(wallet_url).port)), 2000);

      const auto& v = call(wallet_url, kdncd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success kdncd responds with empty object
   ...
   sign->set_callback([&] {
      ...
      if( str_chain_id.size() == 0 ) {
         ilog( "grabbing chain_id from noddnc" );
         auto info = get_info();
   ...
   actionsSubcommand->set_callback([&] {
      ...
      } dnc_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data",data))
   ...
   trxSubcommand->set_callback([&] {
      ...
      } dnc_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_push))
   ...
   trxsSubcommand->set_callback([&] {
      ...
      } dnc_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trxsJson))
   ...
   propose_action->set_callback([&] {
      ...
      } dnc_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",requested_perm))
      fc::variant transaction_perm_var;
      try {
         transaction_perm_var = json_from_file_or_string(transaction_perm);
      } dnc_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm))
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string(proposed_transaction);
      } dnc_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      ...
      } dnc_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } dnc_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            dnc_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "dncio.msig", "propose", result.get_object()["binargs"].as<bytes>()}});
   });
   ...
   review->set_callback([&] {
      auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                         ...
                         ("table", "proposal")
                         ("table_key", "")
                         ("lower_bound", dncio::chain::string_to_name(proposal_name.c_str()))
                         ("upper_bound", "")
                         ("limit", 1)
                         );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      } dnc_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",perm))
      auto arg = fc::mutable_variant_object()
         ("code", "dncio.msig")
         ("action", action)
         ("args", fc::mutable_variant_object()
      ...
      send_actions({chain::action{accountPermissions, "dncio.msig", action, result.get_object()["binargs"].as<bytes>()}});
   };
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            dnc_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      auto result = call(json_to_bin_func, arg);
      send_actions({chain::action{accountPermissions, "dncio.msig", "cancel", result.get_object()["binargs"].as<bytes>()}});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            dnc_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "dncio.msig", "exec", result.get_object()["binargs"].as<bytes>()}});
      }
   );

   // system subcommand
   auto system = app.add_subcommand("system", localized("Send dncio.system contract action to the blockchain."), false);
