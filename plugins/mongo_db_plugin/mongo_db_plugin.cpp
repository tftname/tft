 ...
 */
#include <dncio/mongo_db_plugin/mongo_db_plugin.hpp>
...
public:
   ...
   static abi_def dnc_abi; // cached for common use
                ...
                const chain::action& msg)
  {
     ...
     try {
        ...
        if (msg.account == chain::config::system_account_name) {
           abi = chain::dncio_contract_abi(abi);
        }
        abis.set_abi(abi);
        ...
        try {
           ...
           elog("Unable to convert dnc JSON to MongoDB JSON: ${e}", ("e", e.what()));
           elog("  dnc JSON: ${j}", ("j", json));
        }
     } catch (fc::exception& e) {
...
// For now providing some simple account processing to maintain dnc_balance
void mongo_db_plugin_impl::update_account(const chain::action& msg) {
   ...
   if (msg.name == transfer) {
      ...
      abi_serializer abis;
      auto dncio_account = find_account(accounts, msg.account);
      auto abi = fc::json::from_string(bsoncxx::to_json(dncio_account.view()["abi"].get_document())).as<abi_def>();
      abis.set_abi(abi);
      ...
      asset from_balance = asset::from_string(from_account.view()["dnc_balance"].get_utf8().value.to_string());
      asset to_balance = asset::from_string(to_account.view()["dnc_balance"].get_utf8().value.to_string());
      ...
      update_from << "$set" << open_document << "dnc_balance" << from_balance.to_string()
                  << "updatedAt" << b_date{now}
                  << close_document;
      document update_to{};
      update_to << "$set" << open_document << "dnc_balance" << to_balance.to_string()
                << "updatedAt" << b_date{now}
                << close_document;
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The queue size between noddnc and MongoDB plugin thread.")
         ("mongodb-uri,m", bpo::value<std::string>(),
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database 'dnc' is used if not specified in URI.")
         ;
}
...
void mongo_db_plugin::plugin_initialize(const variables_map& options)
{
   if (options.count("mongodb-uri")) {
      ...
      wlog("dncio::mongo_db_plugin configured, but no --mongodb-uri specified.");
