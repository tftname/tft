 ...
 */
#include <dncio/wallet_plugin/wallet_plugin.hpp>
...
void wallet_plugin::set_program_options(options_description& cli, options_description& cfg) {
   cfg.add_options()
         ...
         ("dncio-key", bpo::value<std::string>(),
          "dncio key that will be imported automatically when a wallet is created.")
         ;
}
...
void wallet_plugin::plugin_initialize(const variables_map& options) {
   ...
   if (options.count("dncio-key")) {
      std::string dncio_wif_key = options.at("dncio-key").as<std::string>();
      wallet_manager_ptr->set_dncio_key(dncio_wif_key);
   }
}
} // namespace dncio
