...
#include <dncio/net_plugin/net_plugin.hpp>
#include <dncio/net_plugin/protocol.hpp>
...
using namespace dncio::chain::plugin_interface::compat;
...
namespace dncio {
   ...
   using dncio::chain::transaction_id_type;
   ...
   struct update_request_time {
      ...
      void operator () (struct dncio::peer_block_state &bs) {
         bs.requested_time = time_point::now();
      }
   } set_request_time;

   typedef multi_index_container<
      dncio::peer_block_state,
      indexed_by<
         ordered_unique< tag<by_id>, member<dncio::peer_block_state, block_id_type, &dncio::peer_block_state::id > >,
         ordered_unique< tag<by_block_num>, member<dncio::peer_block_state, uint32_t, &dncio::peer_block_state::block_num > >
         >
      > peer_block_state_index;


   struct update_known_by_peer {
      void operator() (dncio::peer_block_state& bs) {
         bs.is_known = true;
      }
      void operator() (transaction_state& ts) {
...
//      dispatcher->bcast_transaction(md->packed_trx);
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"dnc Test Agent\""), "The name supplied to identify this node amongst the peers.")
