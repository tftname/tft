 ...
 */
#include <dncio/producer_plugin/producer_plugin.hpp>
...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      uint32_t _production_skip_flags              = 0; //dncio::chain::skip_nothing;
      ...
      fc::microseconds                                          _kdncd_provider_timeout_us;
...
void new_chain_banner(const dncio::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ...
         ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value({std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)}, std::string(default_priv_key.get_public_key()) + "=KEY:" + std::string(default_priv_key)),
          ...
          "Where:\n"
          "   <public-key>    \tis a string form of a vaild dncIO public key\n\n"
          "   <provider-spec> \tis a string in the form <provider-type>:<data>\n\n"
          "   <provider-type> \tis KEY, or KdncD\n\n"
          "   KEY:<data>      \tis a string form of a valid dncIO private key which maps to the provided public key\n\n"
          "   KdncD:<data>    \tis the URL where kdncd is available and the approptiate wallet(s) are unlocked")
         ("kdncd-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
          "Limits the maximum time (in milliseconds) that is allowd for sending blocks to a kdncd provider for signing")
         ;
...
static producer_plugin_impl::signature_provider_type
make_kdncd_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   auto kdncd_url = fc::url(url_str);
   std::weak_ptr<producer_plugin_impl> weak_impl = impl;

   return [weak_impl, kdncd_url, pubkey]( const chain::digest_type& digest ) {
      auto impl = weak_impl.lock();
      if (impl) {
         fc::variant params;
         fc::to_variant(std::make_pair(digest, pubkey), params);
         auto deadline = impl->_kdncd_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_kdncd_provider_timeout_us : fc::time_point::maximum();
         return app().get_plugin<http_client_plugin>().get_client().post_sync(kdncd_url, params, deadline).as<chain::signature_type>();
      } else {
...
{ try {
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            } else if (spec_type_str == "KdncD") {
               my->_signature_providers[pubkey] = make_kdncd_signature_provider(my, spec_data, pubkey);
            }
   ...
   my->_kdncd_provider_timeout_us = fc::milliseconds(options.at("kdncd-provider-timeout").as<int32_t>());
...
{ try {
   ...
   if (!my->_producers.empty()) {
      ...
      if (my->_production_enabled) {
         ...
         //_production_skip_flags |= dncio::chain::skip_undo_history_check;
      }
   }
