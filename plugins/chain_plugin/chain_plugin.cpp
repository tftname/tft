...
#include <dncio/chain/fork_database.hpp>
#include <dncio/chain/block_log.hpp>
...
#include <dncio/chain/reversible_block_object.hpp>

#include <dncio/chain/dncio_contract.hpp>

#include <dncio/utilities/key_conversion.hpp>
#include <dncio/utilities/common.hpp>
...
using namespace dncio::chain::config;
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<dncio::chain::wasm_interface::vm_type>()->value_name("wavm/binaryen"), "Override default WASM runtime")
 ...
 * string contract: contract name (eg: System, dncio.token)
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      genesis_state gs; // Check if dncIO_ROOT_KEY is bad
   } catch( const fc::exception& ) {
      elog( "dncIO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
            ("root_key", genesis_state::dncio_root_key) );
      throw;
   }
   ...
   if( options.count("extract-genesis-json") ||  options.at("print-genesis-json").as<bool>() ) {
      ...
      dnc_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
   }
   ...
   if( options.at("delete-all-blocks").as<bool>() ) {
      ...
      dnc_THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
   ...
   my->chain_config->genesis = fc::json::from_file(genesis_file).as<genesis_state>();
    ...
    load_contract_code_abi("dncio.token", my->chain_config->genesis.token_code, my->chain_config->genesis.token_abi);
    load_contract_code_abi("dncio.bios", my->chain_config->bios_code, my->chain_config->bios_abi);
    load_contract_code_abi("dncio.msig", my->chain_config->msig_code, my->chain_config->msig_abi);
...
read_only::get_info_results read_only::get_info(const read_only::get_info_params&) const {
   ...
   return {
      dncio::utilities::common::itoh(static_cast<uint32_t>(app().version())),
      db.get_chain_id(),
      db.head_block_num(),
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   dnc_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   dnc_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}

read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   const abi_def abi = dncio::chain_apis::get_abi( db, p.code );
   ...
   dnc_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
}
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_table<key_value_index, by_scope_primary>(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      dnc_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      dnc_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( dncio::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_table<key_value_index, by_scope_primary>(p.code, scope, N(stat), [&](const key_value_object& obj){
      dnc_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
static fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis ) {
   ...
   dnc_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));

   const auto* const table_id = db.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(dncio), N(dncio), N(global)));
   dnc_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   dnc_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const {
   const abi_def abi = dncio::chain_apis::get_abi(db, N(dncio));
   const auto table_type = get_table_type(abi, N(producers));
   const abi_serializer abis{ abi };
   dnc_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));

   const auto& d = db.db();
   const auto lower = name{p.lower_bound};

   static const uint8_t secondary_index_num = 0;
   const auto* const table_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(dncio), N(dncio), N(producers)));
   const auto* const secondary_table_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(dncio), N(dncio), N(producers) | secondary_index_num));
   dnc_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   } dnc_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))

   dnc_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   ...
   dnc_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   ...
   try {
      ...
      } dnc_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   const auto& code_account = db.db().get<account_object,by_name>( N(dncio) );
   //const abi_def abi = get_abi( db, N(dncio) );
   abi_def abi;
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   dnc_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi );
      auto action_type = abis.get_action_type(params.action);
      dnc_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
         result.binargs = abis.variant_to_binary(action_type, params.args);
      } dnc_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      dnc_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } dnc_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
